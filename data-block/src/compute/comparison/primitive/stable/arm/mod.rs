...
macro_rules! cmp_scalar {
    ($ty:ty, $suffix:ident, $inner_macro:ident) => {
        paste::paste! {
            ...
            $inner_macro!([<ge_scalar_ $ty>], $ty, [<vdupq_n_ $suffix>], [<vld1q_ $suffix>], [<vcgeq_ $suffix>], );
...
macro_rules! cmp_scalar_half_word {
    ($func_name:ident,
        ...
        #[target_feature(enable = "neon")]
        #[inline]
        unsafe fn $func_name(lhs: &AlignedVec<$ty>, rhs: $ty, dst: *mut BitStore) {
            if lhs.len == 0 {
                return;
            }
...
mod tests {
    ...
    fn test_equal_scalar_i8() {
        ...
        let rhs = -1;
        cmp_assert!(&lhs, rhs, eq_scalar_i8, &[0x0081]);
        cmp_assert!(&lhs, rhs, ne_scalar_i8, &[!0x0081]);
    }
    ...
    fn test_order_scalar_i8() {
        ...
        let rhs = -1;
        cmp_assert!(&lhs, rhs, gt_scalar_i8, &[0x6346]);
        cmp_assert!(&lhs, rhs, ge_scalar_i8, &[0x63c7]);
        cmp_assert!(&lhs, rhs, lt_scalar_i8, &[!0x63c7]);
        cmp_assert!(&lhs, rhs, le_scalar_i8, &[!0x6346]);
    }
    ...
    fn test_equal_scalar_i16() {
        ...
        let rhs = 0;
        cmp_assert!(&lhs, rhs, eq_scalar_i16, &[0x0021]);
        cmp_assert!(&lhs, rhs, ne_scalar_i16, &[!0x0021]);
    }
    ...
    fn test_order_scalar_i16() {
        let lhs =
            AlignedVec::from_slice(&[0, -333, -56, 211, 5667, 0, -256, -1222, -333, 777, 444, 65]);
        let rhs = 0;
        cmp_assert!(&lhs, rhs, gt_scalar_i16, &[0xe18]);
        cmp_assert!(&lhs, rhs, ge_scalar_i16, &[0xe39]);
        cmp_assert!(&lhs, rhs, lt_scalar_i16, &[!0xe39]);
        cmp_assert!(&lhs, rhs, le_scalar_i16, &[!0xe18]);
    }
    ...
    fn test_equal_scalar_i32() {
        ...
        let rhs = -1;
        cmp_assert!(&lhs, rhs, eq_scalar_i32, &[0x048]);
        cmp_assert!(&lhs, rhs, ne_scalar_i32, &[!0x048]);
    }
    ...
    fn test_order_scalar_i32() {
        let lhs = AlignedVec::from_slice(&[345, 123, -835, -1, 565, -345, -1, 456, -3]);
        let rhs = -1;
        cmp_assert!(&lhs, rhs, gt_scalar_i32, &[0x093]);
        cmp_assert!(&lhs, rhs, ge_scalar_i32, &[0x0db]);
        cmp_assert!(&lhs, rhs, lt_scalar_i32, &[!0x0db]);
        cmp_assert!(&lhs, rhs, le_scalar_i32, &[!0x093]);
    }
    ...
    fn test_equal_scalar_i64() {
        ...
        let rhs = -12;
        cmp_assert!(&lhs, rhs, eq_scalar_i64, &[0x0034]);
        cmp_assert!(&lhs, rhs, ne_scalar_i64, &[!0x0034]);
    }
    ...
    fn test_order_scalar_i64() {
        let lhs =
            AlignedVec::from_slice(&[78, 343, -12, -67, -12, -12, 33, 5687, -456, -1, -1, -1, 0]);
        let rhs = -12;
        cmp_assert!(&lhs, rhs, gt_scalar_i64, &[0x1ec3]);
        cmp_assert!(&lhs, rhs, ge_scalar_i64, &[0x1ef7]);
        cmp_assert!(&lhs, rhs, lt_scalar_i64, &[!0x1ef7]);
        cmp_assert!(&lhs, rhs, le_scalar_i64, &[!0x1ec3]);
    }
}
