...
macro_rules! cmp_float_scalar_v2 {
    ($ty:ty, $suffix:ident) => {
        cmp_float_scalar!("sse4.2", "v2", _mm, $ty, $suffix);
    };
}
    ...
    F: Fn(__m128i, __m128i) -> __m128i,
{
    ...
    // Broadcast rhs to 128 bit
    let mut rhs = _mm_set1_epi64x(rhs);
...
#[target_feature(enable = "sse2")]
#[inline]
unsafe fn cmp_scalar_f32_v2<F>(lhs: &AlignedVec<f32>, rhs: f32, dst: *mut u16, cmp_func: F)
where
...
#[target_feature(enable = "sse2")]
#[inline]
unsafe fn cmp_scalar_f64_v2<F>(lhs: &AlignedVec<f64>, rhs: f64, dst: *mut u16, cmp_func: F)
where
