...
/// Note that this function will be optimized for different target-feature to perform
/// auto-vectorization.
///
/// TBD: Will portable-simd accelerate the speed?
#[inline(always)]
...
pub trait ArrayAddElement: PrimitiveType + Add<Output = Self> {
    ...
    const SELECTIVITY_THRESHOLD: f64;
}
...
macro_rules! impl_array_scalar_arith {
    ({$(($ty:ty, $add_threshold:expr, $sub_threshold:expr, $mul_threshold:expr, $div_threshold:expr, $rem_threshold:expr)),+},
        $add:ident,
        $sub:ident,
        $mul:ident,
        $div:ident,
        ...
        $(
            ...
            where U: RemCast<Self>
            {
                ...
                const SELECTIVITY_THRESHOLD: f64 = $rem_threshold;
            }
        )+
    };
}
...
// FIXME:
// - Tune the threshold parameter. The parameter is generated by the benchmark,
//   compare the ratio of partial / full. Different instructions may have different threshold?
//   These parameter is generated on the m1 chip
//
// - The threshold is too small.... Do we really need it ?
impl_array_scalar_arith!(
    {(i8, 0.013, 0.013, 0.013, 0.05, 0.05),
     (u8, 0.013, 0.013, 0.013, 0.05, 0.05),
     (i16, 0.026, 0.026, 0.026, 0.1, 0.1),
     (u16, 0.026, 0.026, 0.026, 0.1, 0.1),
     (i32, 0.05, 0.05, 0.05, 0.2, 0.2),
     (u32, 0.05, 0.05, 0.05, 0.2, 0.2),
     (i64, 0.1, 0.1, 0.14, 0.3, 0.3),
     (u64, 0.1, 0.1, 0.14, 0.3, 0.3),
     (f32, 0.05, 0.05, 0.05, 0.08, 0.08),
     (f64, 0.1, 0.1, 0.1, 0.16, 0.16 )
    },
    add_scalar_dynamic,
    sub_scalar_dynamic,
    mul_scalar_dynamic,
...
macro_rules! arith_scalar {
    ($func_name:ident, $op:tt, $trait_bound:ident, $transform:path) => {
        #[doc = concat!("Perform `array ", stringify!($op), " scalar` for `PrimitiveArray<T>` with `T`")]
        ///
        /// # Safety
        ///
            ...
            T: $trait_bound,
        {
            ...
            if selection.ones_ratio() < T::SELECTIVITY_THRESHOLD {
                let scalar = $transform(scalar);
                selection.iter_ones().for_each(|index| {
                    *uninitialized.get_unchecked_mut(index) = *array_data.get_unchecked(index) $op scalar;
                });
            } else {
                T::FUNC(array_data, scalar, uninitialized);
            }
        }
    };
}

arith_scalar!(add_scalar, +, ArrayAddElement, std::convert::identity);
arith_scalar!(sub_scalar, -, ArraySubElement, std::convert::identity);
arith_scalar!(mul_scalar, *, ArrayMulElement, std::convert::identity);
arith_scalar!(div_scalar, /, ArrayDivElement, T::new_divisor);
    ...
    T: PrimitiveType,
{
    ...
    const SCALAR_FUNC: unsafe fn(&Bitmap, &PrimitiveArray<T>, T, &mut PrimitiveArray<T>);
}
    ...
    ($op:tt, $symbol:tt, $scalar_func:ident, $trait_bound:ident) => {
        paste::paste! {
                ...
                T: PrimitiveType + $trait_bound,
            {
                ...
                const SCALAR_FUNC: unsafe fn(&Bitmap, &PrimitiveArray<T>, T, &mut PrimitiveArray<T>) = $scalar_func;
            }
        }
    };
}
...
/// Perform `array % scalar` for `PrimitiveArray<T>` with `U`
///
/// # Safety
///
/// - If the `selection` is not empty, `array` and `selection` should have same length.
    ...
    U: RemCast<T>,
{
    ...
    let array_data = array.data.as_slice();
    if selection.ones_ratio() < T::SELECTIVITY_THRESHOLD {
        let scalar = T::new_remainder(scalar.cast());
        selection.iter_ones().for_each(|index| {
            *uninitialized.get_unchecked_mut(index) =
                U::cast_back(*array_data.get_unchecked(index) % scalar);
        });
    } else {
        T::FUNC(array_data, scalar, uninitialized)
    }
