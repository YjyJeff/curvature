...
//   These parameter is generated by the m1 chip
...
macro_rules! selected_arith_scalar {
    ($func_name:ident, $op:tt, $trait_bound:ident, $transform:path) => {
        ...
        pub unsafe fn $func_name<T>(
            ...
            T: $trait_bound,
        {
            debug_assert_eq!(selection.len(), lhs.len());
            dst.validity.reference(&lhs.validity);

            let dst = dst.data.as_mut();
            let uninitialized = dst.clear_and_resize(lhs.len());

            let lhs_data = lhs.data.as_slice();
            if selection.ones_ratio() < T::SELECTIVITY_THRESHOLD {
                let rhs = $transform(rhs);
                selection.iter_ones().for_each(|index| {
                    *uninitialized.get_unchecked_mut(index) = *lhs_data.get_unchecked(index) $op rhs;
                });
            } else {
                T::FUNC(lhs_data, rhs, uninitialized);
            }
        }
    };
}

selected_arith_scalar!(selected_add_scalar, +, ArrayAddElement, std::convert::identity);
selected_arith_scalar!(selected_sub_scalar, -, ArraySubElement, std::convert::identity);
selected_arith_scalar!(selected_mul_scalar, *, ArrayMulElement, std::convert::identity);
selected_arith_scalar!(selected_div_scalar, /, ArrayDivElement, T::new_divisor);
...
/// - No other arrays that reference the `dst`'s data and validity are accessed! In the
/// computation graph, it will never happens
pub unsafe fn selected_rem_scalar<T, U>(
    selection: &Bitmap,
    lhs: &PrimitiveArray<T>,
    rhs: U,
    ...
    if selection.ones_ratio() < T::SELECTIVITY_THRESHOLD {
        ...
        selection.iter_ones().for_each(|index| {
            *uninitialized.get_unchecked_mut(index) =
                U::cast_back(*lhs_data.get_unchecked(index) % rhs);
        });
    } else {
        T::FUNC(lhs.data.as_slice(), rhs, uninitialized)
    }
